...
from (SELECT dr.name || ' (' || string_agg(ds.name, ' and ') || ')' as entry
      FROM dist_entry de
      ...
      where de.tree_element_id = 51223464
      group by dr.name, dr.sort_order
...
FROM dist_entry de
         ...
         join dist_entry_dist_status deds on de.id = deds.dist_entry_status_id
         join dist_status ds on deds.dist_status_id = ds.id
         join tree_element te on de.tree_element_id = te.id
