...
When you publish the tree, we simply set the version to published as before. In the background the old TVE's are removed
if they are in the current tree version, thus cleaning up.

.cleanup logic
----
delete from tree_version_element where version = previous version and emement_id in current version
----
...
NOTE: Since https://www.postgresql.org/docs/9.6/arrays.html[arrays are not sets] we are probably be better off using a simple
join table of Version to TVE.
