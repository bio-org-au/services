...
class AuditService implements WithSql {
    ...
    List<Audit> list(String userName, Timestamp from, Timestamp to, String filter) {
        ...
        if (filter == 'all') {
            tableName = '%'
        } else if (['name', 'author', 'reference', 'instance', 'tree_element', 'comment'].contains(filter)) {
            tableName = filter
        } else {
            return []
        }
...
WHERE action_tstamp_tx > :from 
 ...
 ORDER BY event_id DESC LIMIT 500""", [from: from, to: to, user: userName, tableName: tableName, sessionUser: sessionUser]) { GroovyResultSet row ->
     ...
     * @return
     */
    Map report(Timestamp from, Timestamp to, List<String> things = ['name', 'author', 'reference', 'instance', 'tree_element', 'comment']) {
        Map userReport = [:]
        withSql { Sql sql ->
            sql.withTransaction {
